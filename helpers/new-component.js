...
const DEPTH = OUTPUT.substring(ROOT.length + 1, OUTPUT.length).split('/').length;

function getDepth(DEPTH, fromTop = 0) {
  let parts = [];

  for (let i = 0; i < DEPTH - fromTop; i++) {
    parts.push('..');
  }
